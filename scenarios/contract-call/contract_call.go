...
import (
	...
	"github.com/ethpandaops/spamoor/scenarios/contract-call/contract"
...
type ScenarioOptions struct {
	...
	CallDataLength  uint64 `yaml:"call_data_length"`
}
...
var ScenarioName = "contract-call"
...
var ScenarioDescriptor = scenariotypes.ScenarioDescriptor{
	Name:           ScenarioName,
	Description:    "Call contract functions with different configurations",
...
func (s *Scenario) Flags(flags *pflag.FlagSet) error {
	...
	flags.StringVar(&s.options.ContractAddress, "contract-address", "", "Contract address to call")
	flags.Uint64Var(&s.options.CallDataLength, "call-data-length", ScenarioDefaultOptions.CallDataLength, "Calldata length in bytes")
...
func (s *Scenario) Init(walletPool *spamoor.WalletPool, config string) error {
	...
	if s.options.ContractAddress == "" {
		return fmt.Errorf("must define contract-address")
	} else {
...
func (s *Scenario) Run(ctx context.Context) error {
	...
	for {
		...
		go func(txIdx uint64, lastChan, currentChan chan bool) {
			...
			logger := s.logger
			tx, client, wallet, err := s.CallContract(ctx, txIdx, func() {
				if s.pendingChan != nil {
					time.Sleep(100 * time.Millisecond)
					<-s.pendingChan
				}
...
func (s *Scenario) CallContract(ctx context.Context, txIdx uint64, onComplete func()) (*types.Transaction, *txbuilder.Client, *txbuilder.Wallet, error) {
	...
	loadTestContract, err := contract.NewContract(s.contractAddr, client.GetEthClient())
	if err != nil {
	...
	tx, err := wallet.BuildBoundTx(ctx, &txbuilder.TxMetadata{
		...
		// Generate x KB of arbitrary data
		data := make([]byte, s.options.CallDataLength) // x KB
		for j := range data {
			data[j] = byte(j % 256) // Fill with simple pattern
		}
		s.logger.Infof("### calldata length: %d\n", s.options.CallDataLength)
		return loadTestContract.ProcessCallData(transactOpts, data)
