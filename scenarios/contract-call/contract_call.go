...
type ScenarioOptions struct {
	...
	SLoad           bool   `yaml:"sload"`
	SloadOperations uint64 `yaml:"sload_operations"`
	ModExp          bool   `yaml:"mod_exp"`
	ModExpOps       uint64 `yaml:"mod_exp_operations"`
}
...
func (s *Scenario) Flags(flags *pflag.FlagSet) error {
	...
	flags.BoolVar(&s.options.SLoad, "sload", ScenarioDefaultOptions.SLoad, "Call sload heavy contract methods")
	flags.Uint64Var(&s.options.SloadOperations, "sload-operations", ScenarioDefaultOptions.SloadOperations, "SLoad iterations")
	flags.BoolVar(&s.options.ModExp, "mod-exp", ScenarioDefaultOptions.ModExp, "Call mod-exp heavy contract methods")
	flags.Uint64Var(&s.options.ModExpOps, "mod-exp-operations", ScenarioDefaultOptions.ModExpOps, "Mod-exp iterations")
	return nil
}
...
func (s *Scenario) Init(walletPool *spamoor.WalletPool, config string) error {
	...
	if s.options.SLoad {
		if s.options.SloadOperations == 0 {
			return fmt.Errorf("when in sload mode, must define sload operations iteration")
		} else {
			s.sload = true
			s.sloadOperations = s.options.SloadOperations
		}
	} else if s.options.ModExp {
		if s.options.ModExpOps == 0 {
			return fmt.Errorf("when in mod-exp mode, must define mod-exp operations iteration")
		} else {
			s.modExp = true
			s.modExpOps = s.options.ModExpOps
		}
	}
...
func (s *Scenario) Run(ctx context.Context) error {
	...
	for {
		...
		go func(txIdx uint64, lastChan, currentChan chan bool) {
			...
			var wallet *txbuilder.Wallet
			var err error

			if s.sload {
				tx, client, wallet, err = s.SimpleSload(ctx, txIdx, onComplete)
			} else if s.modExp {
				tx, client, wallet, err = s.ModExp(ctx, txIdx, onComplete)
			} else {
				tx, client, wallet, err = s.CallContract(ctx, txIdx, onComplete)
			}
...
func (s *Scenario) ModExp(ctx context.Context, txIdx uint64, onComplete func()) (*types.Transaction, *txbuilder.Client, *txbuilder.Wallet, error) {
	client := s.walletPool.GetClient(spamoor.SelectClientByIndex, int(txIdx))
	wallet := s.walletPool.GetWallet(spamoor.SelectWalletByIndex, int(txIdx))
	transactionSubmitted := false
	...
	tx, err := wallet.BuildBoundTx(ctx, &txbuilder.TxMetadata{
		...
		s.logger.Infof("### sload operations iteration: %d\n", s.options.SloadOperations)
		return loadTestContract.ExecuteModExpAttack(transactOpts, big.NewInt(int64(s.modExpOps)))
		// return testToken.Test(transactOpts, big.NewInt(0))
	})
	if err != nil {
		return nil, nil, wallet, err
...
func (s *Scenario) SimpleSload(ctx context.Context, txIdx uint64, onComplete func()) (*types.Transaction, *txbuilder.Client, *txbuilder.Wallet, error) {
	client := s.walletPool.GetClient(spamoor.SelectClientByIndex, int(txIdx))
	wallet := s.walletPool.GetWallet(spamoor.SelectWalletByIndex, int(txIdx))
	transactionSubmitted := false
	...
	tx, err := wallet.BuildBoundTx(ctx, &txbuilder.TxMetadata{
		...
		s.logger.Infof("### sload operations iteration: %d\n", s.options.SloadOperations)
		return loadTestContract.TestSimpleSload(transactOpts, big.NewInt(int64(s.sloadOperations)))
		// return testToken.Test(transactOpts, big.NewInt(0))
	})
	if err != nil {
		return nil, nil, wallet, err
